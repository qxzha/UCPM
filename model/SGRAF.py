...
class EncoderSimilarity(nn.Module):
    ...
    def forward(self, img_emb, cap_emb, cap_lens):
        ...
        for i in range(n_caption):
            ...
            expand_sim_all.append(self.sim_eval_w(sim_vec))

        # (n_image, n_caption)
        sim_all = torch.cat(sim_all, 1)
        expand_sim_all = torch.cat(expand_sim_all, 1)

        return sim_all, expand_sim_all
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, scores, max_violation):
        self.max_violation = max_violation
...
class SGRAF(object):
    ...
    def forward_sim(self, img_embs, cap_embs, cap_lens):
        ...
        sims, expand_sims = self.sim_enc(img_embs, cap_embs, cap_lens)

        expand_sims, evidences, sims_tanh = torch.sigmoid(expand_sims), torch.exp(torch.tanh(expand_sims) / 0.1), torch.tanh(expand_sims)
        return sims, expand_sims, evidences, sims_tanh

    def forward_loss(self, sims, epoch = None, **kwargs):
        """Compute the loss given pairs of image and caption embeddings
        """
        if epoch >= 1:
            loss = self.criterion(sims, max_violation=True)
        else:
            loss = self.criterion(sims, max_violation=False)
    ...
    def train_emb(self, images, captions, lengths, ids=None, epoch=None, *args):
        ...
        loss = self.forward_loss(sims,epoch=epoch)
